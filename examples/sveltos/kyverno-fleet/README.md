...
and the gate. Chapter two is the canary: the pilot cluster's record approved
...
One record per cluster, including the management cluster, held the way every
...
Widening a rollout here means approving the next cluster's record. Wave one
...
cluster's record already exists, already addresses its cluster, and already
